...
class SyntheticLoader:
    def __init__(self, device, problem="default", ntasks = None, k_spt=1, k_qry=19):
        self.device = device
        data = np.load(type_2_path(problem, ntasks))
    ...
    def test_d(self, n_tasks, mode="train"):
        task_idcs = self.train_range if mode == "train" else self.test_range
        xs, ys, ws = self.xs[task_idcs], self.ys[task_idcs], self.ws[task_idcs]
        num_examples = xs.shape[1]
        x_spt, y_spt, x_qry, y_qry = [], [], [], []
        for i in range(n_tasks):
